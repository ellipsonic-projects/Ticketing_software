...
export const clientApi = {
  createClient: (data: CreateClientInput, token: string) =>
    apiClient<{ client: Client }>('/clients', { method: 'POST', body: JSON.stringify(data), token }),

  onboardClient: (data: OnboardClientInput, token: string) =>
    apiClient<{ client: Client, project: Project }>('/client-onboarding', { method: 'POST', body: JSON.stringify(data), token }),
  ...
  updateClient: (id: string, data: UpdateClientInput, token: string) =>
    apiClient<{ client: Client }>(`/clients/${id}`, { method: 'PATCH', body: JSON.stringify(data), token }),
  ...
  getClientActivity: (id: string, page: number, pageSize: number, token: string) => {
    ...
    return apiClient<unknown>(`/clients/${id}/activity?${params.toString()}`, {
      method: 'GET',
      token,
    });
  },
