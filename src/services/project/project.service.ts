...
import { CreateProjectInput, UpdateProjectInput, ProjectQuery } from '@/lib/project/project.schema';
  ...
  async createProject(
    ...
    return runInTransaction(async (db) => {
      ...
      if (client.status !== 'ACTIVE') {
        throw new AppError('Cannot create project for an inactive or pending client', 400, 'VALIDATION_ERROR');
      }
  ...
  async updateProject(
    ...
    return runInTransaction(async (db) => {
      ...
      if (data.clientId && data.clientId !== project.clientId) {
        ...
        throw new AppError('Cannot change the client association of an existing project', 400, 'VALIDATION_ERROR');
      }

      // Check Duplicate Name
      if (data.name && data.name !== project.name) {
        const existingProject = await ProjectRepository.findByName(tenantId, project.clientId, data.name);
      ...
      const updatedProject = await ProjectRepository.update(tenantId, id, {
        ...
        supportEmail: data.supportEmail !== undefined ? data.supportEmail || null : project.supportEmail,
        supportPhone: data.supportPhone !== undefined ? data.supportPhone || null : project.supportPhone,
        supportNotes: data.supportNotes !== undefined ? data.supportNotes || null : project.supportNotes,
        supportStartDate: data.supportStartDate !== undefined ? data.supportStartDate ? new Date(data.supportStartDate) : null : project.supportStartDate,
        supportEndDate: data.supportEndDate !== undefined ? data.supportEndDate ? new Date(data.supportEndDate) : null : project.supportEndDate,
        updatedById: actorId,
      });
