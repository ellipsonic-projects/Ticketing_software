...
export class ActivityTimelineService {
   ...
   */
  private formatAction(action: string, before: Record<string, unknown> | null, after: Record<string, unknown> | null): { title: string; description: string; icon: TimelineIcon } {
    switch (action) {
      // Common / General
      case 'CLIENT_CREATED':
        return { title: 'Client Created', description: 'The client profile was created.', icon: 'plus' };
      case 'CLIENT_UPDATED':
        return { title: 'Client Updated', description: 'Client details were modified.', icon: 'edit' };
      case 'CLIENT_ARCHIVED':
        return { title: 'Client Archived', description: 'The client was moved to the archive.', icon: 'trash' };
      case 'PROJECT_CREATED':
        return { title: 'Project Created', description: 'The project was created.', icon: 'plus' };
      case 'PROJECT_UPDATED':
        return { title: 'Project Updated', description: 'Project details were modified.', icon: 'edit' };
      case 'PROJECT_ARCHIVED':
        return { title: 'Project Archived', description: 'The project was moved to the archive.', icon: 'trash' };
      
      // SLA / Configuration
      case 'SLA_POLICY_CREATED':
      case 'SLA_POLICY_UPDATED':
        const oldRes = before?.resolutionTimeMinutes ? Math.round(Number(before.resolutionTimeMinutes) / 60) : null;
        const newRes = after?.resolutionTimeMinutes ? Math.round(Number(after.resolutionTimeMinutes) / 60) : null;
      ...
      case 'BUSINESS_HOURS_UPDATED':
        return { title: 'Business Hours Updated', description: 'The weekly business hours schedule was modified.', icon: 'calendar' };
      
      case 'HOLIDAY_CREATED':
        return { title: 'Holiday Added', description: `Added holiday: ${after?.name || 'Unknown'}.`, icon: 'calendar' };
      case 'HOLIDAY_UPDATED':
        return { title: 'Holiday Updated', description: `Modified holiday: ${after?.name || 'Unknown'}.`, icon: 'edit' };
      case 'HOLIDAY_DELETED':
        return { title: 'Holiday Removed', description: `Removed holiday: ${before?.name || 'Unknown'}.`, icon: 'trash' };

      // Support Status
      case 'SUPPORT_ENABLED':
        return { title: 'Support Enabled', description: 'Support services were resumed.', icon: 'play' };
      case 'SUPPORT_PAUSED':
        return { title: 'Support Paused', description: 'Support services were temporarily paused.', icon: 'pause' };

      default:
        // Fallback for unknown events
        const titleCase = action.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase());
