...
async function getAuditLogsHandler(req: NextRequest, ctx?: RouteContext) {
  ...
  if (user.role === Role.PLATFORM_ADMIN) {
    ...
    whereClause.OR = [{ actorId: user.id }, { entity: 'Ticket', entityId: { in: ticketIds } }];
  ...
  const ticketIds = [
    ...new Set(logs.filter((log) => log.entity === 'Ticket').map((log) => log.entityId)),
  ];
  ...
  const formattedLogs = logs.map((log) => {
    ...
    const ticket = log.entity === 'Ticket' ? ticketMap.get(log.entityId) : null;
    return {
      ...log,
      actor: actor
        ? {
      ...
      ticket: ticket
        ? {
            ...
            reportedByName: `${ticket.reportedBy.firstName} ${ticket.reportedBy.lastName}`.trim(),
